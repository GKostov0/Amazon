...
namespace RPGCharacterAnims
{
    public class RPGCharacterWeaponController : MonoBehaviour
    {
        ...
        public void UnsheathWeapon(EWeapon weapon)
        {
        ...
        private IEnumerator _UnSheathWeapon(EWeapon weapon)
        {
			...
			var currentWeaponType = (EWeapon) animator.GetInteger(AnimationParameters.Weapon);
            ...
            if (weapon.Is2HandedWeapon()) {
				...
				if (currentWeaponType.Is2HandedWeapon()) {
					...
					SetWeaponWithDebug(weapon.ToAnimatorWeapon(), -2, currentWeaponType, EWeapon.Unarmed, Side.Unchanged);
				}
				else {
                    DoWeaponSwitch(( int )AnimatorWeapon.UNARMED, weapon, weapon.ToAnimatorWeapon(), Side.Unchanged, false);
					...
					// Wait for WeaponSwitch() to happen then update Animator.
					yield return new WaitForSeconds(0.75f);
					SetWeaponWithDebug(weapon.ToAnimatorWeapon(), -2, weapon, EWeapon.Unarmed, Side.Unchanged);
				}
			}
        }
		...
		public void SheathWeapon(EWeapon fromWeapon, EWeapon toWeapon)
        {
        ...
        /// <returns>IEnumerator for use with StartCoroutine.</returns>
        public IEnumerator _SheathWeapon(EWeapon weaponToSheath, EWeapon weaponToUnsheath)
        {
            ...
            if (weaponToUnsheath.HasNoWeapon()) {
				...
				if (rpgCharacterController.rightWeapon.HasEquippedWeapon()
					...
					SetWeaponWithDebug(weaponToUnsheath.ToAnimatorWeapon(), -2, EWeapon.Unarmed, EWeapon.Unarmed, Side.Unchanged);
				}
			}
        }
        ...
        public void InstantWeaponSwitch(EWeapon weapon)
		...
		/// <returns>IEnumerator for use with StartCoroutine.</returns>
		/// /// <summary>
		public IEnumerator _InstantWeaponSwitch(EWeapon weapon)
		{
			...
			if (weapon.Is2HandedWeapon()) {
				...
				rpgCharacterController.rightWeapon = EWeapon.Unarmed;
				rpgCharacterController.leftWeapon = EWeapon.Unarmed;
				...
				{ rpgCharacterController.SetIKOn(( EWeapon )animator.GetInteger(AnimationParameters.Weapon)); }
			}
		...
		private void DoWeaponSwitch(int weaponSwitch, EWeapon weapon, AnimatorWeapon animatorWeapon, Side side, bool sheath)
		{
		...
		private void SetWeaponWithDebug(AnimatorWeapon animatorWeapon, int weaponSwitch, EWeapon leftWeapon, EWeapon rightWeapon, Side weaponSide)
		{
        ...
        public IEnumerator _WeaponVisibility(EWeapon weaponNumber, bool visible)
        {
            ...
            var weaponType = (EWeapon)weaponNumber;
			switch (weaponType) { case EWeapon.TwoHandSword: SafeSetVisibility(twoHandSword, visible); break; }
            yield return null;
        }
        ...
        private IEnumerator _SyncWeaponVisibility()
        {
            ...
            var rightWeaponType = (EWeapon)rpgCharacterController.rightWeapon;
            switch (rightWeaponType) { case EWeapon.TwoHandSword: SafeSetVisibility(twoHandSword, true); break; }
        }
    }
}